...
2) Create a snapshot:
```
	dbdctl setup-snapshot /dev/sda1 /.elastio 0
```

This will create a snapshot of the root volume at `/dev/dattobd0` with a backing COW file at `/.elastio`. This file must exist on the volume that will be snapshotted.

3) Copy the image off the block device:
```
	dd if=/dev/dattobd0 of=/backups/sda1-bkp bs=1M
```

`dd` is a standard image copying tool in linux. Here it simply copies the contents of the `/dev/dattobd0` device to an image. Be careful when running this command as it can badly corrupt filesystems if used incorrectly. NEVER execute `dd` with the "of" parameter pointing to a volume that has important data on it. This can take a while to copy the entire volume. See the man page on `dd` for more details.
...
This command requests the driver to move the snapshot (`/dev/dattobd0`) to incremental mode. From this point on, the driver will only track the addresses of blocks that have changed (without the data itself). This mode is less system intensive, but is important for later when we wish to update the `/backups/sda1-bkp` to reflect a later snapshot of the filesystem.
...
```
	dbdctl transition-to-snapshot /.elastio1 0
```
...
The current status of the dattobd driver can be read from the file `/proc/elastio-info`. This is a JSON-formatted file with 2 fields: a version number "version" and an array of "devices". Each device has the following fields:
